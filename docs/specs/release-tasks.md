...
- [x] #110 `closeDriver()` を用意（`lib/neo4j.ts`）。Vercel のサーバーレスではプロセスが凍結されるだけで出番が無く、実際に必要なのはローカル側。`scripts/etl-spike/load.ts` と `verify.ts` が `getDriver()` を使いながらプールを閉じる手段を持たない
