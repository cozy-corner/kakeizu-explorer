...
- [x] #102 レスポンスの CDN キャッシュ。成功レスポンスに `CDN-Cache-Control` を付けた（`lib/api.ts` の `cdnCache()`）。`neighbors` / `path` / `person/[id]` は CDN 1 日 + SWR 7 日、`search` は 1 時間 + SWR 1 日。`/api/health` と 404 / 503 は `no-store`
  ...
  - 不採用: `stale-if-error`。**サポートはされている**（[changelog 2026-02-13](https://vercel.com/changelog/stale-if-error-cache-control-header-is-now-supported)、仕様は [Cache-Control headers](https://vercel.com/docs/caching/cache-control-headers)。未サポートは `proxy-revalidate` のみ。`/docs/caching/cdn-cache` の Limits 節は両者を並べて非サポートと書いたままで古く、専用リファレンス側を正とする）。使わないのは、**効いてほしい局面とキャッシュが空になる局面が一致する**ため
    - Aura が寝るのは 72 時間 DB アクセスが無かったときで、それは API リクエストも来ていなかったということ。同じ期間 CDN にも何も溜まらない。Vercel も「めったにリクエストされないアセットはリージョンキャッシュから追い出されうる」としている
    - キャッシュはリージョン単位なので、閲覧者が別リージョンから来れば前の訪問者のキャッシュは無関係
    - 仮に残っていても返せるのは当たった 1 URL だけで、ノードを 1 つ叩けば別 URL で 503 になる
    - 効くのは traffic のあるサイトでオリジンが瞬間的にコケたときで、このサイトの実際の障害モードではない
...
- [解決] #104 エラーモニタリングは現時点では導入しない（close）。サーバーの 500 は Vercel ダッシュボードの Logs で確認でき（保持は短く通知は無いが見に行けば見える）、Aura 停止の検知・通知は #105 の運用ガード（予防）と重複する。固有の空白はクライアント側クラッシュの不可視性のみで、低トラフィックの個人サイトでリアルタイム通知を先行導入する必要は薄い。実運用で想定外の 500 やクライアントエラーに悩まされたら Sentry 等を再検討する
- [ ] #105 運用ガード（Aura の放置対策）。Free は自動ポーズ・削除があるので、定期アクセスか再構築手順を用意する。コードを書かずに済む
- [解決] #121 Vercel の使用量アラートは設定できるものが無い（close）。CLI で実測したところチーム `kakeizu` は `billingPlan: "hobby"` で、支出上限と 50/75/100% 通知を持つ Spend Management も、閾値をカスタマイズできる on-demand usage notifications も **Pro 限定**（[docs](https://vercel.com/docs/spend-management)）。Terraform provider にも該当リソースは無い。Observability の Usage anomaly は Hobby でも使える可能性があるが、定義が「5 分間の使用量が 24 時間平均 + 4σ」で**急変の検出であり無料枠の消化率の閾値通知ではない**ため目的を満たさない。代わりに、無効化できない critical notification（`Usage increased` / `Usage limit reached`）が Hobby にも届き、そもそも Hobby は超過課金されず制限・停止で効くので「気づかないうちに課金が膨らむ」シナリオが存在しない。Pro に上げるか商用利用に踏み込む（#116）際に再検討する
...
→ 実装ゼロの手段（CDN キャッシュ + Vercel の使用量通知）で代替する。将来、想定外の流量が実際に観測されたら再検討する。

なお当初は代替の片方に「使用量アラートの設定」を想定していたが、#121 の調査で Hobby には設定できるアラートが無いと分かった。実際に効くのは CDN キャッシュと、無効化できない Vercel 標準の usage 通知になる。
