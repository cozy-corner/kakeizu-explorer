...
「渡り歩ける」「2人の経路が出る」という MVP のゴールはこの実データで検証済み。右ペインは Wikipedia
記事の直接埋め込み（iframe）に変更済みで、要約→全文の「無駄クリック」は解消した。残る穴は属性:
スパイクは `qid + label` のみで、正確なタイトル（wikipediaTitle）・生没年・画像が未取得。仕上げに
必要な分だけ補う（きれいに作り直さない。ETL の本番化＝下記「MVP の外」へ）:
- [x] 右ペインを Wikipedia 記事の直接埋め込み（iframe）に変更 — 要約カード＋「全文を読む」リンクを撤去し無駄クリック解消。`/wiki/{label}` がリダイレクト/正規化を吸収するため不一致時も Wikipedia 側の該当ページを表示（`components/ArticlePane.tsx`）
- [ ] スパイクの `fetch.ts`/`load.ts` を最小拡張し `birth, death, image, wikipediaTitle` も取得・ロード（現状 qid+label のみ）
- [ ] `wikipediaTitle` を `search`/`neighbors`/`path` のレスポンスに通し、iframe のタイトルを label→wikipediaTitle に切替（`ArticlePane.tsx` の TODO 解消）

> 注: 右ペインの記事被覆率（PR2 実測）— 日本人コア 95.4%、橋渡し親族 54.5%。橋渡しの約半数は記事が無く、iframe には Wikipedia 側の「ページがありません」が出る。
...
- [x] `closeDriver()` を用意（#110）。ここでは長命ランタイム向けと書いていたが実態は逆で、Vercel のサーバーレスではプロセスが凍結されるだけなので出番が無い。必要なのは短命なローカルスクリプト（`scripts/etl-spike/load.ts` / `verify.ts`）側
