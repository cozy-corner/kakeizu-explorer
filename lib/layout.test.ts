...
test("centerOnlyChildren: the focus's blood-line spouse rides along to the midpoint (#30)", () => {
  ...
  expect(obj(center(input, fam(drawn, g), "FO"))).toEqual({
    F: { x: 0, y: 0 },
    M: { x: 0, y: 46 },
...
// Regression guard for the whole placeNodes → project → spouseRouting pipeline on
// real ego graphs (plus the order-dependent synthetics), replacing the old live-DB
// layout-parity script. lib/layout is pure over dagre's coordinates, so freezing
// that output (fixtures/layout/*.json, regenerated by scripts/gen-layout-fixtures.ts)
// makes this deterministic and CI-runnable. `expected` is a golden snapshot: eyeball
// it when regenerating, then it locks the pipeline against silent drift.
type Fixture = {
  qid: string;
  label: string;
  graph: Graph;
